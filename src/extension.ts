...
function registerCommands(context: vscode.ExtensionContext, treeView: S3TreeView): void {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('aws-s3.OpenS3ExplorerView', (bucket: string, key?: string) => {
			if (!Session.Current) {
			...
			try {
				const node = new S3TreeItem(bucket, TreeItemType.Bucket);
				node.Bucket = bucket;
				S3Explorer.Render(Session.Current.ExtensionUri, node, key);
			} catch (err: unknown) {
				ui.showErrorMessage('OpenS3ExplorerView Error', err as Error);
				ui.logToOutput('OpenS3ExplorerView Error', err as Error);
			}
		})
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('S3TreeView.AskAI', async (node: S3TreeItem) => {
			...
			try {
				...
				await AIHandler.Current.askAI(`How can you help with Bucket:${bucket} Key:${key} ?`);
			} catch (error: unknown) {
