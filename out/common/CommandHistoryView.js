...
exports.CommandHistoryView = void 0;
...
class CommandHistoryView {
    ...
    _getWebviewContent(webview, extensionUri) {
        ...
        const history = CommandHistoryManager_1.CommandHistoryManager.Instance.getHistory().slice().reverse(); // Newest first
        let historyRows = "";
        if (history.length > 0) {
        ...
        webview.onDidReceiveMessage((message) => {
            ...
            switch (command) {
                case "refresh":
                    ...
                    return;
                case "clear":
                    CommandHistoryManager_1.CommandHistoryManager.Instance.clear();
                    this.RenderHtml();
                    return;
                case "export":
    ...
    async _exportHistory() {
        try {
            const history = CommandHistoryManager_1.CommandHistoryManager.Instance.getHistory();
            if (history.length === 0) {
            ...
            ui.showErrorMessage(`Failed to export command history: ${message}`, err);
        }
    }
    dispose() {
...
exports.CommandHistoryView = CommandHistoryView;
//# sourceMappingURL=CommandHistoryView.js.map